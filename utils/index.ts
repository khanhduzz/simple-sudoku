...
export function isValidSudoku(board: string[][]): boolean {
  const n = board.length;
  const row: boolean[][] = Array.from({ length: n }, () => Array(n).fill(false));
  const col: boolean[][] = Array.from({ length: n }, () => Array(n).fill(false));
  const square: boolean[][] = Array.from({ length: n }, () => Array(n).fill(false));
